...
const ContactedCaretakers = () => {
  ...
  useEffect(() => {
    const fetchContactedCaretakers = async () => {
      try {
        ...
        const response = await axios.get('http://localhost:3000/message-caretaker');
  ...
  const handleSendMessage = async () => {
    ...
    try {
      await axios.post('http://localhost:3000/message-caretaker', {
        caretakerId,
        message,
      });
  ...
  return (
    <div className="bg-gradient-to-r from-white to-gray-100 min-h-screen flex">
      <div className="w-1/4 bg-gray-200 overflow-y-auto p-4">
        <h1 className="text-3xl font-semibold mb-6">İletilen Bakıcılar</h1>
            ...
            >
              <div className="p-4">
                <div className="flex justify-between items-center mb-2">
                  <h2 className="text-xl font-semibold">{contactedCaretaker.caretakerInfo.name}</h2>
                </div>
              </div>
            </div>
          );
      ...
      <div className="flex-grow p-8">
        {activeChat ? (
          <div>
            <h2 className="text-2xl font-semibold mb-4">Sohbet Penceresi - {selectedCaretaker.caretakerInfo.name}</h2>
